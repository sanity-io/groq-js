...
type GroqFunction = (args: GroqFunctionArg[], scope: Scope, execute: Executor) => PromiseLike<Value>

export const functions: {[key: string]: GroqFunction} = {
  ...
  count: async function count(args, scope, execute) {
  ...
  defined: async function defined(args, scope, execute) {
    if (args.length !== 1) return NULL_VALUE
  ...
  length: async function length(args, scope, execute) {
    if (args.length !== 1) return NULL_VALUE
  ...
  path: async function path(args, scope, execute) {
    if (args.length !== 1) return NULL_VALUE
  ...
  select: async function select(args, scope, execute) {
  ...
  round: async function round(args, scope, execute) {

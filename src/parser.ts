...
function createExpressionBuilder(recurssion: Set<string> = new Set()): MarkVisitor<ExprNode> {
  const exprBuilder: MarkVisitor<ExprNode> = {
    ...
    func_call(p) {
      ...
      if (customFunction !== undefined) {
        const FUNCTION_DECL_BUILDER = createFunctionDeclarationBuilder(recurssion)
...
function createFunctionDeclarationBuilder(
  ...
  return {
    func_decl(p) {
      ...
      const exprBuilder = createExpressionBuilder(new Set([...recurssion, functionId]))

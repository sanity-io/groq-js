...
t.test('Basic parsing', async (t) => {
  ...
  t.test('Can parse text::embedding', async (t) => {
    const query = `*[_type == "book"] | score(text::embedding("monster"))[0..4] { _id, _score }`
    const tree = parse(query)
    t.matchSnapshot(tree)
  })

  t.test('Can parse text::semanticSimilarity', async (t) => {
    const query = `*[_type == "book"] | score(text::semanticSimilarity("monster"))[0..4] { _id, _score }`
    const tree = parse(query)
    t.matchSnapshot(tree)
  })
})
